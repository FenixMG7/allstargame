  ...
  return (
    <svg viewBox="0 0 110 110" width="80" height="80" className="absolute inset-0 -m-2" style={{ zIndex: 1 }}>
      ...
      {isBonus && [0, 1, 2, 3, 4].map(i => {
        ...
        return <circle key={i} cx={55 + 48 * Math.cos(angle)} cy={55 + 48 * Math.sin(angle)} r="2.5" fill={color}
  ...
  return (
    ...
    >
      ...
      <div className="text-center mt-1.5">
        <p style={{ fontFamily: 'Bebas Neue,sans-serif', color: isBonus ? '#FFD700' : 'white', textShadow: '0 1px 6px rgba(0,0,0,1)', fontSize: 11 }} className="leading-tight">
...
function CoachBadge({ coach, role, delay }: { coach: Coach; role: string; delay: number }) {
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    ...
    <div className="flex flex-col items-center gap-2 transition-all duration-500"
      ...
      <div className="relative">
        <div className="w-16 h-16 rounded-full overflow-hidden border-2 bg-[#1A1A1A] flex items-center justify-center"
          style={{ borderColor: '#E8651A', boxShadow: '0 0 14px rgba(232,101,26,0.4)' }}>
          {coach.photo_url
            ? <img src={coach.photo_url} alt={coach.last_name} className="w-full h-full object-cover object-top" />
            : <span style={{ fontFamily: 'Bebas Neue,sans-serif', color: '#E8651A', fontSize: 20 }}>
                {coach.first_name[0]}{coach.last_name[0]}
              </span>
          }
        </div>
        <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 whitespace-nowrap text-[9px] font-bold text-white px-2 py-0.5 rounded-full"
        ...
        <div className="w-full max-w-sm mb-6 page-enter">
          ...
          <div className="bg-[#141414] border border-[#1E1E1E] rounded-2xl p-5">
            <div className="flex justify-around gap-6">
              {result.headCoach && (
                <CoachBadge coach={result.headCoach} role="PRINCIPAL" delay={600} />
              )}
              {result.assistantCoach && (
                <CoachBadge coach={result.assistantCoach} role="ADJOINT" delay={900} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-3 flex flex-col gap-2">
          ...
          {hasCoaches && (
            <>
              ...
              {result.headCoach && (
                <div className="flex items-center gap-3 p-2.5 rounded-xl border border-[#1E1E1E] bg-[#0A0A0A]">
                  <span className="text-lg w-6 text-center">🧑‍💼</span>
                  <div className="w-9 h-9 rounded-full overflow-hidden bg-[#1E1E1E] flex-shrink-0 flex items-center justify-center">
                    {result.headCoach.photo_url
                      ? <img src={result.headCoach.photo_url} alt={result.headCoach.last_name} className="w-full h-full object-cover" />
                      : <span style={{ fontFamily: 'Bebas Neue,sans-serif', color: 'rgba(232,101,26,0.5)' }} className="text-sm">{result.headCoach.first_name[0]}</span>
                    }
                  </div>
                  <div className="flex-1">
                    <span className="font-semibold text-sm text-white block">{result.headCoach.first_name} {result.headCoach.last_name}</span>
                    <span className="text-white/40 text-xs">Coach Principal</span>
                  </div>
                </div>
              )}
              {result.assistantCoach && (
                <div className="flex items-center gap-3 p-2.5 rounded-xl border border-[#1E1E1E] bg-[#0A0A0A]">
                  <span className="text-lg w-6 text-center">👨‍💼</span>
                  <div className="w-9 h-9 rounded-full overflow-hidden bg-[#1E1E1E] flex-shrink-0 flex items-center justify-center">
                    {result.assistantCoach.photo_url
                      ? <img src={result.assistantCoach.photo_url} alt={result.assistantCoach.last_name} className="w-full h-full object-cover" />
                      : <span style={{ fontFamily: 'Bebas Neue,sans-serif', color: 'rgba(232,101,26,0.5)' }} className="text-sm">{result.assistantCoach.first_name[0]}</span>
                    }
                  </div>
                  <div className="flex-1">
                    <span className="font-semibold text-sm text-white block">{result.assistantCoach.first_name} {result.assistantCoach.last_name}</span>
                    <span className="text-white/40 text-xs">Coach Adjoint</span>
                  </div>
                </div>
              )}
            </>
