  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-white/40">Mise à jour automatique en temps réel</span>
      </div>
      ...
      <div className="bg-[#141414] border border-[#1E1E1E] rounded-xl overflow-hidden">
        <table className="w-full admin-table">
          ...
          <tbody>
            {players.map(p => (
              <tr key={p.id}>
                <td style={{fontFamily:'Bebas Neue,sans-serif'}} className="text-lg text-[#E8651A]">{p.number}</td>
                <td className="font-semibold text-white">{p.first_name} {p.last_name}</td>
                <td className="text-white/50 text-sm">{p.position}</td>
                <td>
                  <button onClick={() => toggleActive(p.id, p.is_active)}
                    ...
                    <span className="absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all"
                      style={{left: p.is_active ? '17px' : '2px'}} />
                  </button>
                </td>
                <td>
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#1E1E1E]">
          <div>
            ...
            <p className="text-white/40 text-sm">All-Star Game — Tableau de bord</p>
          </div>
